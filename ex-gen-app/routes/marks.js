...
router.get("/", (req, res, next) => {
  ...
  db.Markdata.findAll({
    where: { userId: req.session.login.id }, //userIdがログインユーザーのものだけを取り出す
    limit: pnum,
...
router.post("/", (req, res, next) => {
  ...
  db.Markdata.findAll({
    where: {
      ...
      //contentに検索フォームに入力されたテキストが含まれているものを取り出す。
...
router.post("/add", (req, res, next) => {
  ...
  db.sequelize.sync().then(() =>
    db.Markdata.create({
      userId: req.session.login.id, //セッションのloginに保存されているUserモデルからidの値を指定。
